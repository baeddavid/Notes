...
The problem can become difficult in a certain edge case. The edge case is when the root is the node out of bounds. We can handle this with two edge cases. If the value at the root is less than the lower bound then we must consider the possibility that the right subtree of the root has values that are inbetween the bounds.

If the value at the root is greater than the upper bound then we must consider the posssibility that elements in the left subtree has values that are inbetween the bounds.

If the value at the root lies inbetween the bounds then we can say that the left child of the root is the left subtree but trimmed. The same can be done with the right subtree.

#+BEGIN_SRC
TrimTree(root, lower, upper)
    if root == null do
        return null
    // If the value at root is less than the lower bound
    if root.val < lower do
        return TrimTree(root.right, lower, upper)
    // If the value at root is greater than the upper bound
    if root.val > upper do
        return TrimTree(root.left, lower, upper)
    root.left <-- TrimTree(root.left, lower, upper)
    root.right <-- TrimTree(root.right, lower, upper)
    return root
#+END_SRC
*** Average Levels in a Binary Search Tree
Given a non-empty binary tree, return the average value of the nodes on each level in the form of an array or list.

We can solve this problem by using a bfs and averaging the values at each level. The only catch is that we must nest a queue within a queue, because we must remove all nodes from the level. To ensure that the integrity of the traversal is preserved we must create a temporary queue to add the next level of the tree. After averaging the current level, we set queue equal to the temporary queue.
    ...
    while Queue.size() != 0 do
        ...
        while Queue.size() != 0 do
            ...
            if node.right != null do
                temp.add(node.right)
        queue <-- temp
        List.add(currentLevelSum / currentNodeCount)
    return List
#+END_SRC
*** Zig Zag Level Order Traversal
Given a binary tree, return the zigzag level order traversal of its node's values.

The problem structurally is similar to printing a binary tree in level order. However the zigzag traversal makes this slightly different. We must begin traversing the tree left to right, then right to left, and repeating so on. The catch here is the data structure we use to store our nodes. For example if we were to solve this problem similarily to level order traversal we may be inclined to use a queue. However implementing it with a queue would be difficult as we would run into issues of adding the nodes in the correct order when switching traversal directions.

Sometimes we will need to perform FIFO and sometimes we will need to perform FILO. We can get around the complexities of using potentially two unique data structures and use two stacks.

If we are currently traversing the tree level right to left then we can push nodes into a stack from right to left. This ensures that the next levels traversal will be done in right to left. If we are currently traversing the tree right to left then we must push in the nodes right to left. Since the stack data structure inherently flips the order of the items we push into it, a stack would be the ideal data structure.
...
ZigZagTraversal(root)
    ...
    while stack.size() != 0 do
        temp <-- new Stack
        currentTraversal <-- new List
        while stack.size() != 0 do
            current <-- stack.pop()
            currentTraversal.add(current.val)
            if isRight do
                if current.left != null do
                    temp.push(current.left)
                if current.right != null do
...
You need to construct a string consisting of parenthesis and integers from a binary tree adhereing to a preorder traversal. The null node needs to be represented by empty paranthesis pair () and you need to omit all the empty parenthesis pairs that don't affect the one-to-one mapping repaltionship between the string and the original binary tree.

Input: [1,2,3,null,4]
Output: 1(2()(4))(3)

The expected output is a bit confusing and takes sometime to understand exactly what we are expected to return. It helps to understand what our base cases look like.

In an instance of a null root we return an empty string.

In an instance of a binary tree with only one node we return the value of the node and ana empty string.

We do not need to worry if only a left child exists for a node, because marking the empty right child is unnecessary because we are asked to do a preorder traversal.

In the case that the only the right child exists for a given node we must mark the empty left child with markers since the left child must be considered first. This case is particular because we do not need to explicitly handle it.

#+BEGIN_SRC java
public String tree2str(TreeNode t) {
    // If the current root is null we return an empty string
    if(t == null) return "";
    // If the current root is a leaf we return the character
    if(t.left == null && t.right == null) return t.val + "";
    // If the root only has a left child then we return the character in addtion to the left subtree as a string
    if(t.right == null) return t.val + "(" + tree2str(t.left) + ")";
    // If the root either has two children or it only has a right child
...
We can probably do this recursively?
Our base cases would be an empty input and single node input. If we pass a leaf into the function it should return true. If we pass null the results should depend.

In an inverted tree root.right = root.left. So if root.right = null then root.left must also equal null for this case to pass. If root.right is null and root.left is not null then the case fails. So we can return false.

Looks something like this maybe?
#+BEGIN_SRC
// Where A and B are the left and right children of the root.
f(A, B)
    if A = NIL AND B = nil do
        return true
    if A != NIL AND B != nil do
        LeftSubTree <-- f(A.left, B.right)
        RightSubTree <-- f(A.right, B.left)
        if LeftSubTree = true AND RightSubTree = true AND A.val = B.val do
            return true
    return false
#+END_SRC
*** Same Tree
...
If both trees are null then we can return true. In the case where one root is not null but the other is then we must return false. Obviously if the values do not match then we must return false.

We can only return true if both the left subtree and right subtrees are identitcal. We can recursively call same on both the left and right subtrees to see if they are identical. If they are both identical we can return true.
...
We could just throw all the nodes into an array and perform a linear scan on the array looking for the absolute minimum difference between the values. It's a linear time solution with a linear space solution.

Could we do this better in place using the tree?
I mean we could use recursion but the space complexity would be the same because of the all the recursive calls we add to the call stack.
...
This problem took me an embarrasingly long amount of time to solve. The solution is fairly simple. The diameter of a binary tree is essentially the longest distance between any two nodes.

We can find this by finding the depth of the left tree and the right tree and adding one since we need to pass through the root.
#+BEGIN_SRC
...
It was initially a difficult question to visualize. Solving the case of a balanced tree with three nodes helps understand the brute force solution.

    1
   / \
  2   3

Given this tree we can create a linked list from it by saving the right subtree with a pointer. We then append 2 to 1.right. Do not forget to disconnect 1.left as it is still connected to 2. Using the right subtree reference we have saved, we then append the pointer to 2.right.

Solving this basic base case gives us the foundation for the brute force solution. A singular node with no children can be considered a linked list. So we begin with assigning both the left and right subtrees to pointers. We then recursively call the flatten method on them. We then disconnect root.left and then append root.right to the recursively flattened left subtree. We then traverse the tree or the "list" until we reach the end and then append the recursively flattened right subtree to it.
...
This algorithm takes O(n log n) due to the fact that we must traverse to the end of the list to append Right.

The optimal solution is very elegant. If we perform a reverse postorder traversal, you will note that we print the tree in reverse order.

We can take advantage of the reversed traversal by building the list from the bottom up. We use a global pointer to keep track of the list we have constructed so far.

We assign root.right to prev as prev is the pointer that holds the constructed list so far. We need to remember to assign root.left to null to disconnect any potential loops. After assigning root.right to prev, we update prev to the new "head" of the list so that we are able to continue building the list.
